...
// app_activate(cur_date,start_time,product_id,dev_os,activate_type,activate_count) VALUES
// (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE
// cur_date=VALUES(cur_date),start_time=VALUES(start_time),product_id=VALUES(product_id),dev_os=VALUES(dev_os),activate_type=VALUES(activate_type),activate_count=VALUES(activate_count)";
...
//        FlinkKafkaConsumer09<String> kafkaConsumer09 = new FlinkKafkaConsumer09<>(topic, new
// SimpleStringSchema(), properties);
//        KafkaUtil.setOffsets(offset, topic, kafkaConsumer09);
//        TypeInformation<Tuple2<String, AppStartLog>> typeInformation = TypeInformation.of(new
// TypeHint<Tuple2<String, AppStartLog>>() {
//        });
...
//                });
//        DataStream<AppStartLog> outStream = AsyncDataStream.orderedWait(inputStream, new
// RedisRichAsyncFunction(), TIMEOUT, TimeUnit.MILLISECONDS, CAPACITY);
//        DataStream<Map<String, String>> result = outStream.map((MapFunction<AppStartLog,
// Tuple2<String, AppStartLog>>) value -> new Tuple2<>(value.getIsExtend() + "_" + value.getDevOs()
// + "_" + value.getProductId(), value))
//                .returns(typeInformation)
//                .keyBy(0)
//                .window(SlidingProcessingTimeWindows.of(Time.minutes(1), Time.minutes(1),
// Time.hours(-8)))
//                .process(new ProcessWindowFunction<Tuple2<String, AppStartLog>, Map<String,
...
// AppStartLog>> elements, Collector<Map<String, String>> out) throws Exception {
...
//                        record.put("start_time", new DateTime(start).toString("yyyy-MM-dd
// HH:mm:ss"));
//                        record.put("activate_type", s[0]);
//                        record.put("dev_os", s[1]);
//                        record.put("product_id", s[2]);
//                        record.put("activate_count",
// String.valueOf(IterableUtils.toList(elements).size()));
//                        out.collect(record);
//                    }
//                });
//
//        //result.addSink(new MySQLSinkForMap(INSERT_STRING, CONNECT_INFO, USERNAME,
// PD)).name("MySQLSink");
...
//
//        @Override
//        public void asyncInvoke(AppStartLog input, AsyncCollector<AppStartLog> collector) throws
// Exception {
...
//                input.setIsExtend("0");
//            } else {
//                RedisFuture<List<KeyValue<String, String>>> future = ((RedisStringAsyncCommands)
// async).mget(values);
