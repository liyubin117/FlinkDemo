...
/**
 * nc -lk my 9888 100,a 200,b 100,b 200,a nc -lk my 9889
...
public class BroadstateMapUserAction {
    ...
    private static final MapStateDescriptor<Void, Map<Integer, Pattern>> bcStateDescriptor =
            new MapStateDescriptor<>(
                    "patterns", Types.VOID, Types.MAP(Types.INT, Types.POJO(Pattern.class)));
                        ...
                        .map(
                                (String str) ->
                                        new Action(
                                                ...
                                                str.split(",")[1]));
        DataStream<Map<Integer, Pattern>> patterns =
                env.socketTextStream("my", 9889)
                        .map(
                                new MapFunction<String, Map<Integer, Pattern>>() {
                                    @Override
                                    public Map<Integer, Pattern> map(String str) throws Exception {
                                        ...
                                        for (int i = 0; i < arr.size(); i++) {
                                            ...
                                            map.put(
                                                    obj.getInteger("patternId"),
                                                    new Pattern(
                                                            obj.getInteger("patternId"),
                                                            obj.getString("firstAction"),
                                                            obj.getString("secondAction")));
                                        }
                                        return map;
                                    }
                                });
        /* 设置Key的字段，同样用户的数据分到相同的operator里 */
        KeyedStream<Action, Integer> actionsByUser =
                actions.keyBy((KeySelector<Action, Integer>) action -> action.getUserId());

        /* 定制 Broadcast Stream */
        BroadcastStream<Map<Integer, Pattern>> bcedPatterns = patterns.broadcast(bcStateDescriptor);
        /* 连接两个流并应用规则 */
        DataStream<Tuple2<Integer, Pattern>> matches =
                actionsByUser
                        ...
                        .process(
                                ...
                                new KeyedBroadcastProcessFunction<
                                    ...
                                    public void processElement(
                                            Action action,
                                            ReadOnlyContext ctx,
                                            Collector<Tuple2<Integer, Pattern>> out)
                                            throws Exception {
                                        // 得到当前模式
                                        Map<Integer, Pattern> map =
                                                ctx.getBroadcastState(bcStateDescriptor).get(null);
                                        if (map == null) { // 检查是否有当前模式，若没有应取默认值
                                            Pattern pattern = new Pattern();
                                            System.out.println(
                                                    "No Broadcast info, Using default pattern "
                                                            + pattern);
                                            map = new HashMap<>();
                                        ...
                                        if (map != null && prevAction != null) {
                                            ...
                                            if (!map.containsKey(action.getUserId())
                                                    && map.containsKey(0)) {
                                    ...
                                    public void processBroadcastElement(
                                        ...
                                        BroadcastState<Void, Map<Integer, Pattern>> bcState =
