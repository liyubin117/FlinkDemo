...
public class RocketmqE2eTest {
    ...
    private static GenericContainer<?> container = new GenericContainer<>(DockerImageName.parse(DOCKER_IMAGE_NAME)).withExposedPorts(9876);
    DockerComposeContainer composeContainer = new DockerComposeContainer<>(
           new File(RocketmqE2eTest.class.getClassLoader()
    ...
    public void test1() throws MQClientException, MQBrokerException, RemotingException, InterruptedException {
        ...
        producer.shutdown();
        DefaultMQPushConsumer consumer = generateConsumer("group1", nameSrvAddress, ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET, TOPIC, TAG);
