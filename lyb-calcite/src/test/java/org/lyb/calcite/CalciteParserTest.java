...
public class CalciteParserTest {
    ...
    public void testSqlSelect() {
        try {
            sqlParser = SqlParser.create("select id,name,age FROM stu where age<20", config);
    ...
    public void testWhere() throws SqlParseException {
        sqlParser =
                SqlParser.create(
                        "select id,count(1) cnt FROM stu where age>20 or id=10 and a=10 and b=10 group by id",
                        config);
        sqlNode = sqlParser.parseStmt();
        System.out.println(sqlNode + "\n");
        ...
        if (sqlNode.getKind().equals(SqlKind.SELECT)) {
            ...
            //                    .filter(x -> SqlKind.LITERAL.equals(x.getKind()))
            //                    .forEach(x -> System.out.println("过滤小于literal的值:" +
            // x.toString()));
        }
    }
    ...
    public void testSqlToRelConverter() throws SqlParseException {
        ...
        // SqlValidatorUtil.newValidator(SqlStdOperatorTable.instance(), calciteCatalogReader,
        // factory,
